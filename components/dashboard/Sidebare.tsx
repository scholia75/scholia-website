...
const Sidebare = () => {
  ...
  return (
  <aside className='relative w-1/4 border-r h-screen overflow-hidden p-6 max-md:hidden'>
 
    <Logo/>
    <ul className='space-y-2 mt-6'>
        {
            dashboardNavs.map(({name,link,icon},index)=>(
                <li key={index} className={clsx('p-3  rounded-lg hover:bg-primary/5 group transition-all duration-300 ',pathname===link && 'bg-primary/5')} >
                   <Link href={link} className={clsx(' font-medium flex flex-row items-center gap-x-3 group-hover:text-primary',pathname===link ? 'text-primary':'text-neutral-500')}>
